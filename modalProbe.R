map <- function(datatime, hopsMap,mirror) {
  if(is.null(mirror) || mirror[1] == "undefined"){
    showModal(modalDialog(
      title = "Undefined DNS Copy",
      easyClose = TRUE
    ))
  }
  ...
  if (!is.null(found)) {
    ...
    for (i in 1:length(data$id)) {
      if (idResult == data$id[[i]]) {
        mirrorCordMapV4 <- data$mirrorCord[[i]]
    ...
    if (module == 1 && is.element(idResult, data$id)) {
      ...
      for (i in 1:length(data$id)) {
        if (idResult == data$id[[i]]) {
          mirrorCordMapV6 <- data$mirrorCord[[i]]
          for (j in 1:length(data$result[[i]])) {
            from <- append(from, data$result[[i]][[j]]$ip)
            tmin <- append(tmin, data$result[[i]][[j]]$tmin)
            tmax <- append(tmax, data$result[[i]][[j]]$tmax)
        ...
        modalDialog(
          ...
          fluidRow(column(
            ...
            box(
              ...
              renderLeaflet(map(hour, hopsMapv4,mirrorCordMapV4))
            )
          ),
          column(
            6,
            box(
              ...
              renderLeaflet(map(hour, hopsMapv6,mirrorCordMapV6))
            )
          )),
          easyClose = TRUE
        )
