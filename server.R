...
shinyServer(function(input, output, session) {
  ...
  findMirrors <- dget("findMirrors2.R")
  ...
  observeEvent(input$action6x4, {
    ...
    if (grepl("[^[:digit:] + ;]", listIDsProbe) ||
      ...
      if (is.null(values$firstclick) ||
        ...
        values$datav6Download  <- leitura(url,day,month,year)
        
        url <- paste(values$root, "datav4_", values$hour, ".zip", sep = "")
        values$datav4Download  <- leitura(url,day,month,year)
      ...
      if (length(datav4) < 1 || length(datav6) < 1) {
        shinyalert(          
          title = "Error",
          text = "No data found for the chosen parameters.",
          type = "error",
          closeOnClickOutside = FALSE,
          closeOnEsc = FALSE,
        ...
        if (length(datav4$ProbeID) == 0 ||
          ...
          #probesList$as  <- NULL
          teste <<- probesList
          probesList$as <- as.character(probesList$as)
          values$probesList <- probesList
          
          if (length(datav4$ProbeID) > 10 ||
              length(datav6$ProbeID) > 10) {
            datav4 <- datav4[1:10, ] #UNCOMENT
            datav6 <- datav6[1:10, ] #UNCOMENT
          ...
          for (i in probesDisplayed$ProbeID) {
            displayedIDs <- append(displayedIDs, which(values$probesList$ProbeID == i))
          ...
          observeEvent(input$reload, {
            showModal(
              modalDialog(
                ...
                bsButton("attlist6x4", style = "primary", label = "OK"),
                bsButton("clearAttlist",style = "default",label = "CLEAR")
              )
            )
          })
          
          observeEvent(input$attlist6x4, {
            ...
            if (length(listRows) == 0 || is.null(listRows)) {
              ...
              probesDisplayed <- values$probesList[listRows, ]
              datav4 <-
  ...
  observeEvent(input$actionTemp, {
    ...
    if (grepl("[^[:digit:] + ;]", listIDsProbe) ||
      ...
      if (is.null(values$firstclickTemp) ||
        ...
        values$dataTemp <- leitura(url,day,month,year)
      ...
      if (length(data) < 1 || nrow(data) < 1) {
        ...
        if (length(data$ProbeID) == 0) {
          ...
          auxMe$mirrorCord <- data$mirrorCord
          ...
          observeEvent(input$reload, {
            showModal(
              modalDialog(
                ...
                easyClose = TRUE,
                DT::dataTableOutput("groupslist"),
                bsButton("attlistTemp", style = "primary", label = "OK"),
                bsButton("clearAttlist",style = "default",label = "CLEAR")
              )
            )
          })
          ...
          observeEvent(input$attlistTemp, {
            values$listRows <- input$groupslist_rows_selected
            listRows <- values$listRows
            if (length(listRows) == 0 || is.null(listRows) || length(listRows) > 10) {
              sendSweetAlert(session = session,
